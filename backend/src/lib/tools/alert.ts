/**
 * Price alerts — checked every monitor tick.
 */
...
export interface PriceAlert {
  ...
  createdAt: string; triggered: boolean; triggeredAt?: string; note?: string;
}
...
export function setAlert(symbol: string, price: number, direction: 'above' | 'below', note?: string): PriceAlert {
  const a: PriceAlert = {
    id: `a_${Date.now()}`, symbol: symbol.toUpperCase(),
    price, direction, createdAt: new Date().toISOString(),
    triggered: false, note,
  };
  const list = readJson<PriceAlert[]>(FILE, []);
  list.push(a); writeJson(FILE, list);
  return a;
}
...
export function removeAlert(id: string): boolean {
  ...
  if (i < 0) return false;
  list.splice(i, 1); writeJson(FILE, list); return true;
}
...
export async function checkAlerts(): Promise<PriceAlert[]> {
  ...
  for (const s of symbols) prices[s] = await fetchPrice(s);
  ...
  for (const a of active) {
    ...
    if (hit) {
      a.triggered = true; a.triggeredAt = new Date().toISOString();
      triggered.push(a);
